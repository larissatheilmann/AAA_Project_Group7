...
- **⚠ Stale-run warning (added after the zero-mile-trip drop):** the h3_7 SVM-vs-NN figures in this entire subsection are **not currently reproducible**. The latest pipeline re-ran only the SVR on the post-drop data; the FNN predictions in `models_predictions/FNN/` were *not* regenerated. `3.4` builds the comparison by merging SVR and FNN on the actual-demand key `y_true`, so the stale FNN file makes the inner join silently drop every high-demand cell (test max demand collapses 5,177→220) — the merged table and scatter below therefore describe a biased low-demand subset, not real performance. Regenerate the FNN (rerun `3.3`) **and** change the merge key off `y_true` before trusting any number here. Clean SVR **test** skills straight from `3.2b` are **+3.5% MAE (daily)** and **+4.8% MAE (hourly)**; the pre-drop figures in the table below (SVM daily +10.0%, hourly +0.8%) predate the data change.
