...
For this project, the ERA5-Land hourly dataset 1950–present, later referred to as ERA6-Land, was selected as the primary source of meteorological data. It is provided via the Copernicus Climate Data Store (CDS) and produced by the European Centre for Medium-Range Weather Forecasts (ECMWF) as part of the Copernicus Climate Change Service (C3S).
ERA5 is a family of global atmospheric reanalysis products that combines numerical weather prediction models with observational data through data assimilation, resulting in a temporally continuous representation of atmospheric conditions. Unlike raw observational records, ERA5 contains no temporal gaps, as missing observations are reconstructed through the underlying physical model.
Several ERA5 variants exist, each suited to different applications. ERA5-Land was chosen for its higher spatial resolution over land surfaces, which is particularly relevant for city-level analysis such as this project's focus on Chicago.
The dataset was selected based on three criteria directly relevant to the client's needs. First, its gap-free structure removes the need for imputation, which is critical for building reliable demand-forecasting models. An incomplete weather record would directly translate into blind spots in predicted ride demand and fleet requirements, thus generating need for a handling of this missing data. Second, the data is freely accessible, well-documented, and widely validated in scientific and industry applications, ensuring the analysis is reproducible and auditable by the client's own teams. Third, since this analysis is restricted to a single city, ERA5's gridded structure can be simplified to a single coordinate-based time series, keeping the data pipeline lightweight and easy to maintain in production.

#### Selected Variables
Six meteorological variables were selected for their expected relevance to ride-hailing demand and electric vehicle (EV) fleet operations:
- 2m temperature (K): Air temperature at 2 meters above the surface. Expected to drive both customer demand (e.g., fewer walk/bike substitutes in extreme cold) and EV battery performance, since range might drop at low temperatures.
- Total precipitation (m): Accumulated liquid and solid precipitation reaching the surface. Expected to increase short-distance ride demand and reduce walking/cycling alternatives.
- 10m u/v wind components (m/s): Eastward and northward horizontal wind components at 10 meters above ground level. Is combined into wind speed and direction (see Feature Engineering), relevant for extreme-weather demand spikes and outdoor-wait discomfort.
- Snow cover (0–1) and snow depth (m): Fraction of the grid cell covered by snow and mean snow thickness on the ground (excluding canopy snow). Expected to directly affect road conditions, trip duration, and safety-related demand drops, and are key drivers of charging infrastructure planning in winter months.
Together, these variables capture thermal conditions, atmospheric dynamics, and precipitation effects. The three weather dimensions we think have the most direct operational impact on an electric ride-hailing fleet.

#### Data Quality and Preprocessing
A small number of precipitation and snow cover values were marginally below zero. As negative values are physically impossible for these variables, which is attributable to minor rounding errors in the underlying computations. Those values were clipped to a lower bound of zero. For interpretability, the temperature was also converted from Kelvin to Celsius and the precipitation to millimeters, aligning with standard meteorological reporting conventions.

#### Dataset Overview
The final dataset comprises 17,544 hourly observations across 9 columns: a timestamp, the six raw meteorological variables, and two engineered variables (wind speed and wind direction; see Feature Engineering). No missing values are present, consistent with the reanalysis nature of the data.
Exploratory analysis (Figure X) confirms clear seasonal patterns, most notably in temperature, precipitation, and snow-related variables. Snow cover and snow depth peak between November and March/April and fall to near zero in summer. This directly aligns with the seasonal fleet-utilization patterns discussed in Section Y. Snow cover approaches 100% precisely when snow depth peaks and temperatures are lowest, confirming internal consistency of the data. Precipitation shows no strong correlation with snow variables, as expected given the distinct physical processes behind rain and snowfall.
No anomalies or implausible extremes were found: temperatures range from below −30°C to roughly 35°C, precipitation stays below ~20 mm/hour, and snow depth does not exceed 30 cm. All of this is consistent with Chicago's historical climate. A comparison against long-term climatological averages (Figure X) further supports the plausibility of the dataset. For the client, this means the weather inputs feeding the demand model are trustworthy and require no additional cleaning before being used in forecasting.
...
#### Executive Summary
An automated charging controller was designed and evaluated for the client's future ride-hailing EV fleet. Two reinforcement learning methods were implemented and compared: Tabular Q-Learning and a Deep Q-Network (DQN). Across five independent training runs, Tabular Q-Learning reduced the average daily charging cost by approximately 22% relative to a naive strategy of always charging at full power, while the probability of a vehicle running out of energy mid-shift remained statistically indistinguishable from that baseline.
DQN was also evaluated, but its results vary considerably across training runs and it is not recommended for deployment at this stage. It might increase in relevance once fleet size or state complexity increases drastically as well. All results were obtained from five independent and reproducible training runs per method.

#### Problem Framing & Methodology

##### Problem Description
Each vehicle in the ride-hailing fleet returns home at 2:00 PM and must depart again at 4:00 PM for that day's shift. During this two-hour window, an automated controller decides, every 15 minutes, how fast to charge the battery, selecting from four available charging speeds. This is comparable to a limited number of modes offered by chargers.
Two factors act in opposite directions: electricity prices increase later in the afternoon, closer to departure, reflecting typical time-of-day grid pricing, and the energy required for the day's shift is not known until departure and varies from day to day around a typical value of 30 kWh.
A vehicle that runs out of energy mid-shift results in a missed fare and a stranded driver, which is substantially more costly than a marginally higher electricity bill. The controller must balance these two considerations for every vehicle, every day.

##### Formalizing the Problem
The problem was formalized as a Markov Decision Process (MDP) to make it solvable with reinforcement learning. At each time step, the model observes the current battery level and the remaining time until departure, and selects one of four charging speeds (0, 7.33, 14.67, or 22 kW). Each decision incurs a small electricity cost, and a penalty of $500 is applied if the battery level is insufficient to meet demand at departure.
The penalty was set substantially larger than the maximum achievable savings from smart charging (at most a few tens of cents per day), so that the model prioritizes reliability over marginal cost savings.
Two further modeling choices affect the results. First, the battery level on arrival is drawn from a uniform distribution between 0% and 50% of capacity for each simulated day. This approach has been chosen over assuming the battery to be always empty, reflecting variation in real driver behavior and a more realistic setting. Tthis also introduces a cost-risk trade-off, since less charging is required on days when the vehicle already retains more charge. Second, the per-kWh price coefficient increases from approximately 7 to 11 cents across the eight time steps, reflecting a simplified time-of-day pricing structure and creating an incentive to shift charging toward earlier, cheaper time steps where this does not increase risk.

##### Solution Methods
Two reinforcement learning methods were implemented and compared, as suggested in the project brief. Tabular Q-Learning maintains a lookup table estimating, for each combination of battery level and remaining time, the expected value of each charging action. This is well suited to the present problem, which has a small, enumerable number of states. The DQN approach replaces this lookup table with a neural network capable of generalizing across similar states, which becomes relevant once the number of possible states is too large to tabulate. For example, if the model were later extended.
Testing both methods allows an assessment of whether the added complexity of a neural-network-based approach is currently justified for this problem, or whether the simpler tabular method performs comparably or better.

##### Validation Approach
A single training run can produce misleading conclusions purely by chance, depending on the random conditions encountered during training. To address this, each method was trained independently five times, using different random seeds, and the resulting policies were evaluated over several thousand simulated days each. Differences between methods were assessed using e.g. the Ø Failure rate. Two additional experiments were conducted to justify specific hyperparameter choices empirically which can be found in the notebook for more details.

#### Results

##### Tabular Q-Learning
Figure 1 tracks training progress over 150,000 simulated days. The training score increases and then stabilizes, and the probability of running out of energy decreases sharply before settling at a low, stable level. This pattern indicates that the model converges to a consistent policy rather than continuing to change unpredictably.
...
Figure 2 shows the resulting policy: for each combination of remaining time and battery level, it indicates the charging speed selected by the model. The two intermediate charging speeds are used across a substantial share of states, rather than only the extremes of zero and full power, indicating that the policy differentiates between situations rather than applying a single fixed rule.
...
##### Deep Q-Network
The corresponding training curves for DQN (Figure 3) show a markedly different pattern: rather than stabilizing, both the training score and the failure rate continue to oscillate throughout the full 20,000-episode training period.

![Figure 3a. DQN training score.](../plots/learning_curve_dqn.png){#fig-bild}
![Figure 3b. DQN failure rate during training.](../plots/failure_curve_dqn.png){#fig-bild}

As a consequence, the network's performance at the end of training is not necessarily representative of its best achieved performance during training. To obtain a usable model despite this instability, the best-performing set of weights observed at any point during training was retained, rather than the final weights. This step is important as without the deployed policy could have been substantially worse than what training was capable of producing. Increasing the number of episodes did not yield any benefits here either, which is why, in contrast to the Q-table approach, the number of episodes is significantly lower (20,000 vs. 150,000). Furthermore, this allows for a reduction in resources in terms of the execution time required.
The retained policy (Figure 4) charges at full power in almost all observed states, differing from the naive baseline in only two narrow regions of the state space. This suggests that, for the configuration tested, DQN largely reproduces the naive strategy rather than identifying a materially different policy.
