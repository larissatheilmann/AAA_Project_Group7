...
**Support Vector Machine — shortfalls & results (fully drafted, use nearly as-is)**:
- **Headline finding**: SVR barely beats, and at one level loses to, a historical-mean baseline. Skill score ranges from **+10.2%** (h3_7/daily, best) to **−25.8%** (census/hourly, worst); community/hourly is a near coin-flip (+1.3%)
- Daily beats hourly at every matching spatial resolution: h3_7 (+10.2% vs +5.8%), census (+6.3% vs −25.8%), community (+9.0% vs +1.3%)
- Feature-importance methods disagree only at the two coarsest daily levels (census/daily, community/daily) — `base_demand` looks critical by permutation importance but nearly free to drop by ablation there; everywhere else the reliance is genuine (+215.9% to +248.4% MAE cost if ablated)
- The predictive ceiling is largely inherent to the assignment's non-autoregressive framing, not a fixable modeling gap
- Residual-on-baseline reframing improves 5 of 6 levels (census/hourly closes over half its gap; community/hourly flips to a clear win); one level (census/daily) marginally worse, traced to validation-selection variance
- Kernel choice empirically justified: all 6 winning models selected RBF — a linear kernel's unbounded log-space predictions explode through the `expm1` back-transform
- Improvement levers for follow-up: weather×hour/spatial interactions, finer/spatially-resolved weather, richer POI features (distances, not just counts); also worth testing natural-prevalence training given the balanced-sample/test-prevalence mismatch
...
3. Prediction accuracy: SVR barely beats baseline (+10.2% to −25.8%); NN comparison pending
